...
class CTCRecognizer(object):
    ...
    def SE_Moudle(self,inputs):
        ...
        with tf.variable_scope("SE_Moudle"):
            ...
            moudle = slim.convolution2d(moudle,num_outputs=num_channels,
    ...
    def CTC_block(self,inputs,out_channals,name):

        with tf.variable_scope(name):
            branch1 = slim.convolution2d(inputs, num_outputs=out_channals / 2, kernel_size=[1,1],stride=1)
            branch1 = slim.convolution2d(branch1,num_outputs=out_channals / 4, kernel_size=[3,3],stride=1)
            branch1 = self.SE_Moudle(branch1)
            branch1 = slim.convolution2d(branch1,num_outputs=out_channals / 2, kernel_size=[3, 1], stride=1)
            branch1 = slim.convolution2d(branch1,num_outputs=out_channals / 2, kernel_size=[1, 3], stride=1)

            branch2 = slim.convolution2d(inputs,num_outputs=out_channals / 2, kernel_size=[1,1],stride=1)

            block_output=slim.convolution2d(branch1+branch2,
                                            num_outputs=out_channals / 2, kernel_size=[1, 1],stride=1)
        return block_output


    def inference(self,inputs,seq_length):

        print("CTCRecognizer_new3")
        #inputs:(?,60,30,3)
        ...
        with slim.arg_scope(self.CTC_arg_scope(is_training=self.is_training,
            ...
            net = self.CTC_block(net,128,"CTC_block1_1")

            net1 = slim.max_pool2d(net, kernel_size=[3, 3], stride=[2, 1])
            net1 = self.CTC_block(net1, 256,"CTC_block1_2")
            net1 = slim.max_pool2d(net1, kernel_size=[3, 3], stride=[1, 2])
            net1 = slim.dropout(net1,keep_prob=self.keep_prop)
            net1 = self.res_DW_block(net1,256,stride=1)
            net1 = slim.dropout(net1, keep_prob=self.keep_prop)
            # print("net1:",net1)     #(?, 30, 15, 128)

            net2 = slim.convolution2d(inputs, 64, kernel_size=[3, 3], stride=1)
            net2 = slim.max_pool2d(net2, kernel_size=[3, 3], stride=1)
            net2 = self.res_DW_block(net2, 128, stride=2)
            net2 = slim.max_pool2d(net2, kernel_size=[3, 3], stride=1)
            net2 = self.res_DW_block(net2, 128, stride=1)
            net2 = slim.max_pool2d(net2, kernel_size=[3, 3], stride=1)
            net2 = slim.dropout(net2,keep_prob=self.keep_prop)
            # print("net1:",net1)     #(?, 30, 15, 128)

            net=tf.concat([net1,net2],axis=3,name="concat")
            net = slim.convolution2d(net, self.num_classes*2, kernel_size=[1, 4], stride=1)
            net = slim.convolution2d(net, self.num_classes, kernel_size=[1, 4], stride=1)
            # logits = tf.reduce_mean(net, axis=2)

            logits = slim.avg_pool2d(net, kernel_size=[1, 15], stride=[1, 15])
            print("logits0:", logits)
